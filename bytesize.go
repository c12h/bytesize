// Package bytesize lets you output "17Kib", "3.45MiB", "67.8GiB" etc when
// printing numbers of byte.
//
// It exports type ByteSize, which is a wrapper for int64.  Therefore (1)
// ByteSize values can be negative, and (2) int64(x) returns the underlying
// value of x.
//
// When printing a ByteSize value with a format string, %#v generates what
// fmt.Sprintf("ByteSize(%d)", int64(value)) would.  Using %v (without a '#'
// flag) generates strings like "1B", "2KiB", "3.45MiB", "67.8GiB", "987TiB",
// "1023EiB" etc.  In either case, the other conversion flags are ignored,
// except that '-' and a width specifier have the usual effect.
//
// If any other verb is used, the effect is the same as on the underlying value.
// Note that fmt.Print(), fmt.Println() etc use %v on every operand.
//
// When %v is used without the hash flag, the output is up to 4 decimal digits,
// perhaps with a decimal point between two of those digits.  In this case,
...
//
// ByteSize has a String() method, which always uses the default suffixes.  For
// completeness, ByteSize also has a GoString method, which is equivalent to
// formatting the value with "%#v".
//
// This package exports only one directly-visible name, ByteSize.  When using
// this package, you may want to define a type alias, like this:
...
// The String() method returns what formatting the value with "%v" (or "%.3v")
// would: strings like "0B", ..., "1023B", "1KiB", ..., "1.01Kib", "-23.4MiB",
// "340GiB" etc.
//
// This method makes ByteSize satisfy the fmt.Stringer interface.  (Because
// ByteSize also satisfies the fmt.Formatter interface, printing ByteSize values
// will use .Format(…) rather than this method.)
//
...
// ByteSize also satisfies the fmt.Formatter interface, fmt will print ByteSize
// values using .Format(…), not this method.)
//
...
//
// For verbs other than %v, it simply applies the same conversion specifier to
// the underlying int64 value.
//
// For %#v, it outputs fmt.Sprintf("ByteSize(%d)", int64(n)).
//
// For %v without "#", it outputs a decimal number with 3 or 4 significant
// digits followed by the appropriate suffix.  The output may or may not have a
// decimal point.  As mentioned in the package documentation, a precision
// specifier format verb affects which suffixes are used, not how many digits
// are output.
//
func (b ByteSize) Format(f fmt.State, verb rune) {
	...
	var output []byte
	if f.Flag('#') {
		output = []byte(fmt.Sprintf("ByteSize(%d)", nBytes))
	} else {
		prec, specified := f.Precision()
		if !specified || prec > 3 {
			prec = 3
		} else if prec < 0 { // ‘cannot happen’, but cope anyhow
			prec = 0
		}
...
func equivalentFormat(f fmt.State, verb rune) string {
	...
	formatString += string(verb)
	return formatString
}

// formatByteSize does the hard work for this package.
//
// Note that it returns a byte slice, not a string.
